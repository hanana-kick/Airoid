...
class AirPlayService : LifecycleService(), RaopCallbackHandler, LogListener {
    ...
    /** 현재 세션의 페어링 코드. 프로세스당 한 번 생성되며 앱 언어가 바뀌면 재생성된다. */
    ...
    fun startServer() {
        ...
        // 페어링 코드: 프로세스 수명 동안 유지 — 서버 재시작이나 연결 해제로는 바뀌지
        // 않고, 앱을 완전 종료했다 다시 켜면 새 코드를 뽑는다. 기기 식별(영속 MAC/페어링
        // 인증서)은 별개 값이라 코드가 바뀌어도 클라이언트는 같은 기기로 인식한다.
        // 단, 앱 언어가 바뀌면 그 언어로 새로 생성해 갱신한다.
        val appLocale = resources.configuration.locales[0] ?: Locale.getDefault()
        var code = sessionPairingCode
        if (code == null || sessionPairingLang != appLocale.language) {
            code = PairingCode.random(appLocale)
            sessionPairingCode = code
            sessionPairingLang = appLocale.language
        }
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        // 스와이프로 앱 작업을 지우면 완전 종료로 본다. FGS라 프로세스가 바로 죽지 않고
        // 캐시로 남을 수 있으므로, 세션 페어링 코드를 여기서 폐기해야 다음 실행 때
        // 같은 프로세스라도 새 코드를 뽑는다.
        if (_serverState.value == ServerState.RUNNING) stopServer()
        sessionPairingCode = null
        sessionPairingLang = null
    ...
    companion object {
        ...
        // 페어링 코드는 프로세스(앱 실행) 수명 동안만 유지된다 — 완전 종료 후 재실행하면
        // 새로 뽑는다. 서비스 인스턴스가 재생성돼도 같은 프로세스면 같은 코드를 쓴다.
        private var sessionPairingCode: String? = null
        private var sessionPairingLang: String? = null
    }
}
