...
dnssd_register_raop(dnssd_t *dnssd, unsigned short port)
{
    ...
    /* RAOP와 AirPlay를 같은 이름으로 노출해 클라이언트에서 한 기기로 보이게 한다 */
    strncpy(dnssd->raop_servname, dnssd->name, sizeof(dnssd->raop_servname) - 1);
    dnssd->raop_servname[sizeof(dnssd->raop_servname) - 1] = '\0';
